...
RUN apk --no-cache add git git-fast-import python2 vim && \
	wget -q -O /usr/bin/git-p4.py https://raw.githubusercontent.com/git/git/v2.33.0/git-p4.py && \
	wget -q -O /usr/bin/p4 https://cdist2.perforce.com/perforce/r21.1/bin.linux26x86_64/p4 && \
