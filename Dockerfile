...
RUN apk --no-cache add git git-fast-import python2 vim && \
	wget -O /usr/bin/git-p4.py https://raw.githubusercontent.com/git/git/v2.30.1/git-p4.py && \
	wget -O /usr/bin/p4 http://cdist2.perforce.com/perforce/r20.2/bin.linux26x86_64/p4 && \
